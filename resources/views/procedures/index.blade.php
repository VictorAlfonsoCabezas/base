@extends('layouts.app')
@section('title')Procesos @stop
@section('breadcrumbs1')Procesos @stop
@section('breadcrumbs2')Procesos @stop
@section('custom_css') @stop
@section('content')
<div class="panel panel-inverse">
    <!-- BEGIN panel-heading -->
    <div class="panel-heading">
        <h4 class="panel-title">Lista de Procesos</h4>
    ...
    <div class="panel-body">
        <table id="table-procedures" class="table table-striped table-bordered align-middle">
            <thead>
                <tr>
                    <th width="1%">#</th>
            ...
            <tbody>
                ...
                <tr id='{{$procedure->id}}'>
                    <td width="1%">{{$procedure->id}}</td>
                    <td>{{$procedure->company->company_name}}</td>
                    <td>{{$procedure->date_created}}</td>
                    <td>
                        <b>{{$procedure->name}}</b>
                    </td>
                    <td class="text-center">
                        <span class="badge bg-warning rounded-pill">{{$procedure->code_intel}}</span>
                    </td>
                    <td class="text-center">
                        @if($procedure->status)
